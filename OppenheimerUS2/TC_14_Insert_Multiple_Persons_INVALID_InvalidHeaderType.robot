...
*** Test Cases ***
TC_14:Insert_Multiple_Persons_INVALID_InvalidHeaderType
    [Tags]  Functional
    ...
    ${header}=      Create Dictionary   Content-Type=${content_type_xml}
    ${response}=    POST On Session    session    ${endpoint_insertMultiple}  json=${list}    headers=${header}     expected_status=${status_code_unsupportedMediaType}

    #Validations
    ${status_code}=     convert to string   ${response.status_code}
    Should Be Equal    ${status_code}    ${status_code_unsupportedMediaType}